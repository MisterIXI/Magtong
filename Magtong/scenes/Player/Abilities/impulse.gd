...
func _ability_button_down():
	if cd_timer.is_stopped() and player.state != player.polarity.IDLE:
		cd_timer.start(settings.impulse_cooldown)
		for puck in map.pucks:
			...
			var scaled_force = (
				(puck.global_position - player.global_position).normalized() * force * settings.magnet_force * mult * settings.impulse_mult
			)
...
func emit_pulse(pulse_position: Vector2, pol: PlayerBody.polarity):
	for puck in map.pucks:
		...
		var force = settings.magnet_dropoff.sample(dist / settings.magnet_range)
		var scaled_force = ((puck.global_position - pulse_position).normalized() * force * settings.magnet_force * mult * settings.impulse_mult)
